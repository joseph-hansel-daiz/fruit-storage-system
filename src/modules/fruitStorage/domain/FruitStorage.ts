...
export class FruitStorage {
    ...
    public static create(name: string, description: string, limitOfFruitToBeStored: number, amountInStorage: number = 0): FruitStorage {
        const fruitName = FruitName.create(name);
        const fruitDescription = FruitDescription.create(description);
        return new FruitStorage(fruitName, fruitDescription, limitOfFruitToBeStored, amountInStorage);
    }
}
